...
ローカル（Jaeger）は **オリジンを差し替え可能な形** で設定済み:
...
2. **同一オリジンproxy（推奨）**: webapp に `/api/otel/v1/traces` 等の Route Handler
   を置き、ブラウザはそこへ送る（同一オリジンなので CORS 不要）。サーバ側で内部
   コレクタへ転送する。コレクタを公開せず済むため本番で安全。
   - `PUBLIC_OTEL_EXPORTER_OTLP_ENDPOINT` をこの proxy パスに向けるだけで切替できる。
...
webapp コンテナに
`PUBLIC_OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318/v1/traces` を設定済み
（ブラウザはホストから Jaeger の OTLP/HTTP に到達）。Jaeger UI では Service
`webapp-browser` として確認できる。
