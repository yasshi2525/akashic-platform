...
クライアントはプレイヤーのブラウザに対応するパッシブインスタンスと、 `akashic-server` 上で実行されるアクティブインスタンスの2種類です。
...
### `getTickList`, `getStartPoint` の省リソース化

長時間起動ゲームの場合に送信データ量が多くなりがちな `getTickList`, `getStartPoint` 呼び出し時に、データを分割して送受信できるよう追加のイベント定義をしています。
